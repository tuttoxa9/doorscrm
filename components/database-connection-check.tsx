...
export function DatabaseConnectionCheck() {
  ...
  const getStatusIcon = () => {
    switch (connectionStatus.status) {
      case "connected":
        return <Wifi className="h-5 w-5 text-green-500 dark:text-green-400" />
      case "disconnected":
        return <WifiOff className="h-5 w-5 text-red-500 dark:text-red-400" />
      case "checking":
  ...
  const getStatusBadge = () => {
    switch (connectionStatus.status) {
      case "connected":
        return (
          <Badge variant="outline" className="bg-green-50 dark:bg-green-950/50 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800 animate-pulse">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1 overflow-hidden">
            <div
              className={cn(
                ...
                connectionStatus.status === "connected" && "w-full bg-green-500 dark:bg-green-400",
                connectionStatus.status === "disconnected" && "w-full bg-red-500 dark:bg-red-400",
                connectionStatus.status === "checking" && "w-1/2 bg-blue-500 dark:bg-blue-400 animate-pulse",
              )}
            />
          </div>
        </div>
