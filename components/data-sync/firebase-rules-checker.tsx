...
export function FirebaseRulesChecker() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "success":
        return <Badge className="bg-green-500/20 dark:bg-green-500/30 text-green-600 dark:text-green-400 border-green-500/30 dark:border-green-500/50">Успешно</Badge>
      case "error":
        return <Badge variant="destructive" className="bg-red-500/20 dark:bg-red-500/30 text-red-600 dark:text-red-400 border-red-500/30 dark:border-red-500/50">Ошибка</Badge>
      default:
        ...
        {tests.length > 0 ? (
          <div className="space-y-3">
            {tests.map((test, index) => (
              <div key={index} className="flex items-center justify-between p-3 border rounded bg-card hover:bg-accent/50 transition-colors">
